  ...
  if (env.JWT_SECRET === "your_jwt_secret_key") {
    ...
    try {
      ...
      if (finalSecret && finalSecret.length >= 32) {
        console.warn(`Loaded persisted JWT_SECRET from ${secretPath}`);
      }
      else {
        finalSecret = crypto.randomUUID();
        await Bun.write(secretPath, finalSecret);
        console.warn(`Generated new random JWT_SECRET and saved to ${secretPath}`);
      }

      env.JWT_SECRET = finalSecret;
    }
    catch (error) {
      console.error(`Failed to auto-generate secret key in production (${secretPath}):`, error);
      process.exit(1);
    }
  }
}
